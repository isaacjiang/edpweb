...
export class SettingsComponent {
    ...
    initialiazation(current_user,menuID) {
      ...
      this.toggleFunction(current_user,menuID)
    ...
    toggleFunction = function (current_user,menuId) {
      let root = this

    // console.log($scope.selectedFunc)
    if (menuId == 'settings1') {

      let url = "/api/syssetting/getsettings"+"?params=all"
      this.api.get(url).subscribe((resp)=>{
        console.log(resp)
        root.formTitle = ["Description","Actions"]
        root.formData = resp
      })
      //
